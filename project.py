...
def scrape_head_line_news():
    print("[헤드라인 뉴스]")
    url="https://news.naver.com/"
    soup=create_soup(url)
    news_list=soup.find("ul", attrs={"class":"hdline_article_list"}).find_all("li", limit=3) # limit으로 숫자 제어 가능
...
def scrape_english():
    ...
    print("(영어지문)")
    for sentence in sentences[len(sentences)//2:]: # 8문장이 있다고 가저앟ㄹ때, 4~7까지 잘라서 가져옴
        print(sentence.get_text().strip())
    print()
    print("(한글지문)")
    for sentence in sentences[:len(sentences)//2]: # 8문장이 있다고 가저앟ㄹ때, 0~3까지 잘라서 가져옴
        print(sentence.get_text().strip())

if __name__== "__main__":
    scrape_weather() # 오늘의 날씨 정보 가져오기
    scrape_head_line_news() # 헤드라인 뉴스 정보 가져오기
    scrape_it_news() 
